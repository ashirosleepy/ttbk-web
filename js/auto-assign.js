...
function fairDistribute(items, load, presetAssignments = {}) {
...
async function askAIForAssignment(items, load) {
  ...
  try {
    ...
    const resp = await fetch(endpoint, {
      ...
      body: JSON.stringify({
        tasks: items,
        members: STATE.profiles.map((p) => ({ id: p.id, name: p.name })),
        current_load: load,
      }),
    });
    ...
    result.assignments.forEach((a) => {
      if (!a || !a.title) return;
      const validMember = STATE.profiles.some((p) => p.id === a.assigned_to);
      if (validMember) {
